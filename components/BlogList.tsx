...
function BlogList({posts}: Props) {
  return (
    <div>
        <hr className="border-[#F7AB0A] mb-18" />
        <div className="grid grid-cols-1 md:grid-cols-2 px-10 gap-x-10 gap-y-16 pb-24">
            {posts.map(post =>(
                <ClientSideRoute key={post._id} route={`/post/${post.slug.current}`}><div key={post._id} className="group cursor-pointer flex flex-col">
